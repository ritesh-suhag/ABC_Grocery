...
In the project, we assisted the ABC grocery in creating 2 different machine learning models - 
1. Predicting Customer Loyalty Scores
2. Enhancing Customer Targeting

While the models work well, it has no value to the business till the users can use it. Thisi s where model deployment comes into play. Deploying ML models as an API service enables the business to use the model at will and get the most value out of it. In this section we will walk-through on take the model from *jupyter notebook* and deploy it online.

### Actions <a name="overview-actions"></a>

Deployment usually the last step in any Data Science Project lifecycle, to be able to integrate your ML/DL model to a web application is quite an important task. The first step in deploying the model is ensuring data consistency. What it really means is, when we get the raw data, we have to ensure that we apply the same pre-processing steps that we did before training the data. 

The data should be in the exact format as required by the model including handling missing values, categorical variables, feature scaling, feature selection and feature engineering steps. This can be achieved easily by creating a pipeline to make all the changes. A pipeline acts like a blackbox which takes in the raw data and returns a clean and formatted data which can be consumed by our model.

The next step is to save the final trained model as a pickle object so that it can be accessed easily along with the created pipeline. We can then load the pickle obeject in our API and use the predict method to get the prediction. 

### Results <a name="overview-results"></a>

The goal of this project is to wrap the model in an API. The API loads the pipeline and the model saved to make the prediction. The input to the API is sent as a JSON object. FastAPI can also be used along with the 'pydantic' package which has a module 'BaseModel' which helps to define the input being sent in to the API. 
...
According to techempower, an independent website that benchmarks web servers by running a variety of tests on them, FastAPI+uvicorn is one of the fastest web servers.
...
FastAPI allows validating user inputs by adding constraints on string and numerical query parameters.
...
Before creating our ML model lets start by creating a basic API that’s going to return us a simple message. With just one line of code, we can initialize a FastAPI instance. This app object is responsible for handling the requests for our REST API. 
...
For demonstration we will train the model for 'Predicting Customer Loyalty Scores' and save the appropriate requried objects (the pipeline object and the model) to make predictions using the API. 
...
Although in the 'Basic API' section we used the GET method, while building APIs for ML models, we use POST method. This is because it's considered better practice to send parameters in JSON rather than in URL.
...
**Below steps take place in the API -**

1. The pipeline model is unpickled and and saved as 'pipeline_regressor'. This model object will be used to get the predictions.
...
This plarticular API and all APIs for this application are hosted on Heroku and can be accessed at - [API Deployment](https://ml-api-ritesh.herokuapp.com/docs)
